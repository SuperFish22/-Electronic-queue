...
def login():
    ...
    if request.method == "GET":
        ...
        if login == "":
            return render_template("errorscreen.html", error_name = "No login", error_message = "Попробуйте другой логин")
        if pwd == "":
            return render_template("errorscreen.html", error_name = "No password", error_message = "Попробуйте другой пароль")
        ...
        try:
            ...
            return render_template("errorscreen.html", error_name = "Пользователь не найден", error_message = "Пользователь не найден. Проверьте логин и пароль")
...
def monitor(number):
    if request.method == "GET":
        """Ссесия["username"] != "monitor" or session["userid"] != number:
            return render_template("errorscreen.html", error_name = "Access denied", error_message = "You do not have access to this page")"""
