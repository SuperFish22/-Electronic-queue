...
def save_queue_item(number, username, birthdate_str, operation):
    birthdate_obj = datetime.strptime(birthdate_str, '%Y-%m-%d')
    new_item = QueueModel( number=number, username=username, operation=operation , birthdate=birthdate_obj)
...
def pers(json):
    ...
    save_queue_item(client_number, username, birthdate, operation)
    with app.app_context():
        queue = get_queue_items()
    queue_data = [{'number': item.number, 'username': item.username, 'birthdate': item.birthdate.strftime('%Y-%m-%d'), 'operation': item.operation,'status': item.status} for item in queue]
